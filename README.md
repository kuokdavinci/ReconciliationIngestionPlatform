...
API stream nhiều trang có thể stage raw pages trong MongoDB GridFS trước khi review. `sourceUnitKey` explicit được ưu tiên; nếu không có, runtime dùng fetch metadata hash. Replay dùng cùng identity để retry an toàn; ordered backfill dùng một `backfillRunId` và resume theo ngày.
...
| 3A | EDA, provenance, frozen baseline, controlled mutation và coverage handoff | Implemented |
...
Phase 2 cố ý không biến fraud label, amount outlier, coordinate hay entity
consistency thành automatic rejection. Đây vẫn là observation/monitoring hoặc
business contract cho đến khi có acceptance riêng.

### Runtime outcome quan trọng

- `CONTINUE`/`INGESTED`: row hợp lệ được ghi và checkpoint được advance.
- `REVIEW`/`CONTINUE`: row lỗi được quarantine, row hợp lệ vẫn chạy tiếp.
- `REVIEW`/`HOLD_FOR_REVIEW`: conflict cần operator xử lý trước khi tiến checkpoint.
- `WAITING_REVIEW`: stream đã stage đủ nhưng chờ mapping/operator approval.
- `FAILED`/`BLOCKED`: lỗi cấu hình hoặc runtime cần recovery; không retry vô hạn.
- `SAFE_DUPLICATE`: source stream đã hoàn tất, lần chạy lại không fetch/ghi lại.
