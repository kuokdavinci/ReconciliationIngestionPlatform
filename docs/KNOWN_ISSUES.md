...
- Next.js optimized build dùng Webpack (`next build --webpack`); không đổi sang Turbopack nếu chưa có verification tương đương.

## Airflow pilot

- Compose pilot đang manual-only với `AIRFLOW_GLOBAL_SCHEDULE=none`.
...
- Recovery hardening đã có live ViettelPay retry/recovery và current-image VNPAY
  ...
  concurrent isolation và rollback evidence vẫn mở.
...
- `run.py --data/--config` vẫn có trong CLI nhưng local-file ingestion chưa được compose repository hoàn chỉnh; flow vận hành hiện tại dùng API/Airflow. Không dùng hai flag này cho Compose demo.
...
- `mongo-express` trong Compose tắt basic auth cho local convenience; chỉ bind local.
