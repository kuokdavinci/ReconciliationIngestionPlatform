  ...
  return Array.isArray(samplePreview)
    ? samplePreview.slice(0, 5).map((sample, index) => {
        ...
        return {
          ...
          values: Object.fromEntries(
            values.slice(0, 5).map((value, valueIndex) => [`Column ${valueIndex + 1}`, value == null ? null : String(value)])
          ),
        };
      })
    : [];
...
export function normalizeRuntimeValidation(packet: Record<string, unknown>): RuntimeValidationResult | null {
...
export function normalizeScopeRecommendation(packet: Record<string, unknown>): ReviewScopeRecommendation {
...
export function normalizeMappingConstraints(packet: Record<string, unknown>): MappingConstraint[] {
