...
| `#review-queue` | Review Queue | Approval desk with right-side packet drawer, validation gates, approve/reject/send-to-studio actions |
...
| `GET /api/v1/mappings` | Review Queue, Studio | List mapping configs by partner |
...
| `GET /api/v1/review-packets` | Review Queue | List pending/historic packets |
| `GET /api/v1/review-packets/{id}` | Review Queue | Packet detail |
| `POST /api/v1/review-packets/{id}/approve-activate` | Review Queue | Approve + activate |
| `POST /api/v1/review-packets/{id}/approve-keep-current` | Review Queue | Approve keep current |
| `POST /api/v1/review-packets/{id}/reject` | Review Queue | Reject |
| `POST /api/v1/review-packets/{id}/send-to-studio` | Review Queue → Studio | Handoff to studio |
...
| `POST /api/v1/mapping/ai-generate` | Studio / Review Queue | AI field mapping from sample |
