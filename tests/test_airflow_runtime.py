...
def test_business_date_treats_naive_timestamp_as_utc(monkeypatch) -> None:
    from src.config.settings import settings

    monkeypatch.setattr(settings, "business_timezone", "Asia/Ho_Chi_Minh")

    assert business_date(datetime(2026, 8, 8, 17, 0)) == date(2026, 8, 9)
